        ...
        client.connect(uri, { useUnifiedTopology: true }, async function (err, db) {
            if(err){
                ...
                try {
                    ...
                    console.log('[ ' + chalk.greenBright('OKAY') + ' ]' + ' Bookend shows added to database.  Continuing....');
                    await db.close();
                    ...
                    if (err.message === 'Collection already exists. NS: pos-schedule-films.films') {
                 ...
                 client.connect(this.uri,  { useUnifiedTopology: true }, async function (err, db) {
                    if(err){
                        ...
                        try {
                            ...
                            reject({ exit_code: 1, error: err, message: 'An error ocurred sending database record(s).'});
                        }
